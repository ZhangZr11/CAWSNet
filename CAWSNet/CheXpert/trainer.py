...
import random
from CheXpert.tools import CheXpertDataSet
from Res2Net.res2net_experiment import  res2net50_26w_4s_ex
...
pathFileTrain = '/media/ubuntu/19069210-0923-4248-82e6-e6fb4cad2fb9/CheXpert-v1.0-small/train.csv'
pathFileValid = '/media/ubuntu/19069210-0923-4248-82e6-e6fb4cad2fb9/CheXpert-v1.0-small/valid.csv'
...
# transformList.append(transforms.RandomResizedCrop(imgtransCrop))
# transformList.append(transforms.RandomHorizontalFlip())
transformList.append(transforms.RandomRotation(degrees=5, resample=False, expand=False, center=None))
transformList.append(transforms.ColorJitter(contrast=0.1, saturation=0.1, hue=0.1))
...
dataLoaderVal = DataLoader(dataset=datasetValid, batch_size=trBatchSize, shuffle=False, num_workers=20, pin_memory=True)
dataLoaderTest = DataLoader(dataset=datasetTest, batch_size=trBatchSize, shuffle=False,num_workers=20, pin_memory=True)
...
class CheXpertTrainer():
    def train(model, dataLoaderTrain, dataLoaderVal, nnClassCount, trMaxEpoch, launchTimestamp, checkpoint):
        ...
        for epochID in range(0, trMaxEpoch):
            ...
            batchs, losst, losse = CheXpertTrainer.epochTrain(model, dataLoaderTrain, optimizer, trMaxEpoch,
                                                              nnClassCount, loss = get_loss_function)
            lossVal = CheXpertTrainer.epochVal(model, dataLoaderVal, optimizer, trMaxEpoch, nnClassCount, loss = get_loss_function)
            # a, b, auc = CheXpertTrainer.test(model, dataLoaderTest, nnClassCount, checkpoint=None,
            #                                  class_names=class_names)
            print("########################################验证集AUC###############################")
            c, d, auc_vaild = CheXpertTrainer.test(model, dataLoaderVal,nnClassCount,checkpoint = None, class_names = class_names)
            print("########################################测试集AUC###############################")
            a, b, auc_test = CheXpertTrainer.test(model, dataLoaderTest, nnClassCount, checkpoint=None,
                                             class_names=class_names)
            # model, dataLoaderTest, nnClassCount, checkpoint, class_names
            timestampTime = time.strftime("%H%M%S")
            timestampDate = time.strftime("%d%m%Y")
            ...
            if lossVal < lossMIN:
                ...
                torch.save({'epoch': epochID + 1, 'state_dict': model.state_dict(), 'best_loss': lossMIN,
                           ...
                           'm-epoch' + str(epochID) + '-loss-' + launchTimestamp + '.pth.tar')
                print('Epoch [' + str(epochID + 1) + '] [best-loss-save] [' + timestampEND + '] loss= ' + str(lossVal))
            else:
                print('Epoch [' + str(epochID + 1) + '] [----] [' + timestampEND + '] loss= ' + str(lossVal))
            if(auc_vaild>auc_vaild_MAX):
                auc_vaild_MAX = auc_vaild
                torch.save({'epoch': epochID + 1, 'state_dict': model.state_dict(), 'best_auc': auc,
                            'optimizer': optimizer.state_dict()},
                           'm-epoch' + str(epochID) + '-valid-auc-' + str(auc_vaild) + launchTimestamp + '.pth.tar')
                print('Epoch [' + str(epochID + 1) + '] [valid-auc-save] [' + timestampEND + '] auc= ' + str(auc_vaild))
            if (auc_test > auc_test_MAX):
                auc_test_MAX = auc_test
                torch.save({'epoch': epochID + 1, 'state_dict': model.state_dict(), 'best_auc': auc,
                            'optimizer': optimizer.state_dict()},
                           'm-epoch' + str(epochID) + '-test-auc-' + str(auc_test) + launchTimestamp + '.pth.tar')
                print('Epoch [' + str(epochID + 1) + '] [test-auc-save] [' + timestampEND + '] auc= ' + str(auc_test))
    ...
    def epochTrain(model, dataLoader, optimizer, epochMax, classCount, loss):
        ...
        for batchID, (varInput, target) in enumerate(dataLoaderTrain):
            ...
            # print(type(varTarget))
            # print(type(varInput))
            # print(varInput.size())
            # print(varTarget.size())
            varInput = torch.autograd.Variable(varInput)
            varTarget = torch.autograd.Variable(varTarget)
            # varTarget = target.cuda()


            varOutput = model(varInput)
            # print(varTarget.size())
            # print(varOutput.size())
            ...
            if batchID % 200 == 0:
                print(batchID // 200, "batches computed")
                ...
                print(str(batchID) + '  Training loss ：' + str(l) + '  Batch loss ：' + str(le))
                # print()
                # print()
...
model = res2net50_26w_4s_ex(pretrained=True).cuda()
